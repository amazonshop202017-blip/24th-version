          ...
          >
            ...
            <div className="absolute top-3 right-3 z-10 flex items-center gap-2">
              <Select 
                ...
                onValueChange={(val) => {
                  const newTagId = val === "none" ? "" : val;
                  updateScreenshotTag(selectedScreenshot.id, newTagId);
              ...
              <button
                onClick={() => {
                  removeScreenshot(selectedScreenshot.id);
            ...
            {/* Tag badge on image */}
            {getTagById(selectedScreenshot.tagId) && (
              <div 
                className="absolute top-3 left-3 z-10 px-2 py-0.5 rounded text-xs font-medium text-white"
                style={{ backgroundColor: getTagById(selectedScreenshot.tagId)?.color }}
              >
                {getTagById(selectedScreenshot.tagId)?.name}
              </div>
            )}
